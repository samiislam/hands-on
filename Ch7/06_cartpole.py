...
@torch.no_grad()
def unpack_batch(batch: list[ExperienceFirstLast], net: Net, gamma: float) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    states = []
    actions = []
    rewards = []
    done_masks = []
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(buffer) < 2*BATCH_SIZE:
            continue
        batch = cast(list[ExperienceFirstLast], buffer.sample(BATCH_SIZE))
        states_v, actions_v, tgt_q_v = unpack_batch(batch, cast(Net, tgt_net.target_model), GAMMA)
        optimizer.zero_grad()
        q_v = net(states_v)
