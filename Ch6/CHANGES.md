...
> **Off-policy bias.** N-step DQN is off-policy: transitions sit in a replay buffer and the bootstrap target uses a greedy max-Q. The n-step return, however, sums intermediate rewards that were collected under the *behavior* policy (epsilon-greedy), not the current greedy policy. This mismatch introduces off-policy bias that grows with `n`: with n=3 maybe 1–2 actions differ from the current policy, but with n=100 almost the entire trajectory is stale and the accumulated return becomes meaningless. Larger `n` also increases variance, and because the bootstrap term is discounted by `γⁿ` it contributes almost nothing at large `n`, losing the stabilizing effect of the learned value function.
>
