...
if __name__ == "__main__":
    ...
    env_factories = [
        lambda: wrap_dqn(gym.make("ALE/Pong-v5", frameskip=1))
    ...
    net = common.AtariA2C(obs_shape, int(act_space.n)).to(device)
    print(net)

    agent = ptan.agent.PolicyAgent(
        lambda x: net(x)[0], apply_softmax=True, device=device)  # type: ignore[arg-type]
    ...
    with common.RewardTracker(writer, stop_reward=19) as tracker:
